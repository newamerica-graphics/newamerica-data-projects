...
class CustomHomegrownMap extends React.Component {
  constructor(props) {
    ...
    let data = props.data[props.vizSettings.primaryDataSheet].filter(
      props.vizSettings.filterInitialDataFunction
    );
  ...
  sendGeomRequests() {
    d3.json(
      "https://na-data-projects.s3.amazonaws.com/geography/us.json",
      (error, data) => {
        let statesGeom = topojson.feature(data, data.objects["states"])
          .features;
    ...
    d3.json(
      "https://na-data-projects.s3.amazonaws.com/geography/world.json",
      (error, data) => {
        let countriesGeom = topojson.feature(data, data.objects.countries)
          .features;
  ...
  updateStatesMap(stateObject) {
    ...
    this.statesProjection = d3
      .geoAlbersUsa()
      .scale((5 * width) / 4)
      .translate([width / 2, height / 2]);

    this.statesPathGenerator = d3.geoPath().projection(this.statesProjection);
  }

  setInitialCountriesMap() {
    this.countriesProjection = d3
      .geoEquirectangular()
      .scale(1)
      .rotate([-12, 0])
      .translate([0, 0]);

    this.countriesPathGenerator = d3
      .geoPath()
  ...
  updateCountriesMap(stateObject) {
    ...
    this.countriesProjection.scale(s).translate(t);
  ...
  componentWillUpdate(nextProps, nextState) {
    ...
    if (
      nextState.statesGeom &&
      nextState.countriesGeom &&
      (!statesGeom || !countriesGeom || nextState.width != this.state.width)
    ) {
      ...
      <div className="custom-homegrown-map">
        <div className="custom-homegrown-map__main" ref="fullContainer">
          <div className="custom-homegrown-map__section">
            <h5 className="custom-homegrown-map__section__title">
              Jihadist Terrorists with Origins in the U.S.
            </h5>
            <div
              className="custom-homegrown-map__section__map-container"
              ref="renderingArea"
            >
                    ...
                    {statesGeom.map(d => {
                      ...
                      return (
                        <path
                          ...
                          fillOpacity={
                            tooltipVal &&
                            tooltipVal.data.geo_type == "state" &&
                            tooltipVal.id === d.id
                              ? ".5"
                              : "1"
                          }
                          onMouseOver={e => {
                            return d.data ? this.mouseover(d, e) : null;
                          }}
                          onMouseOut={() => {
                            return d.data ? this.mouseout(d) : null;
                          }}
                        />
                      );
                    })}
          ...
          <div className="custom-homegrown-map__section">
            <h5 className="custom-homegrown-map__section__title">
              Jihadist Terrorists with Origins Outside the U.S.
            </h5>
                    ...
                    {countriesGeom.map(d => {
                      ...
                      return (
                        <path
                          ...
                          fillOpacity={
                            tooltipVal &&
                            tooltipVal.data.geo_type == "country" &&
                            tooltipVal.id === d.id
                              ? ".5"
                              : "1"
                          }
          ...
          <div className="custom-homegrown-map__legend__section">
            <div className="custom-homegrown-map__legend__section__content">
              <div className="custom-homegrown-map__legend__color-swatch-container">
                <div
                  className="custom-homegrown-map__legend__color-swatch"
                  style={{ backgroundColor: colors.red.light }}
                />
              </div>
              <h5 className="custom-homegrown-map__legend__text">
                Birth State of Jihadist Terrorist Responsible for Attack in U.S.
              </h5>
            </div>
          </div>
          <div className="custom-homegrown-map__legend__section">
            <div className="custom-homegrown-map__legend__section__content">
              <div className="custom-homegrown-map__legend__color-swatch-container">
                <div
                  className="custom-homegrown-map__legend__color-swatch"
                  style={{ backgroundColor: "#927d85" }}
                />
              </div>
              <h5 className="custom-homegrown-map__legend__text">
                Birth State of Jihadist Terrorist Responsible for Attack in U.S.
                and Trump Visa Restricted Country
              </h5>
            </div>
          </div>
          <div className="custom-homegrown-map__legend__section">
            <div className="custom-homegrown-map__legend__section__content">
              <div className="custom-homegrown-map__legend__color-swatch-container">
                <div
                  className="custom-homegrown-map__legend__color-swatch"
                  style={{ backgroundColor: colors.turquoise.light }}
                />
              </div>
              <h5 className="custom-homegrown-map__legend__text">
                Trump Visa Restricted Country*
              </h5>
            </div>
          </div>
        </div>
        <div className="custom-homegrown-map__annotation">
          <h5 className="custom-homegrown-map__annotation__text">
            *On March 6, 2017 the Trump administration issued a new executive
            order, which did not include Iraq in the list of visa restricted
            countries. On September 24, the travel ban was revised again to drop
            Sudan, and add travel restrictions regarding Venezuela and North
            Korea (not displayed on this map), as well as Chad.
          </h5>
        </div>
        {tooltipVal && tooltipVal.data.text && tooltipVal.data.text != "" && (
          <div className="custom-homegrown-map__tooltip" style={tooltipPos}>
            {tooltipVal.data.text}
          </div>
        )}
      </div>
    );
  ...
  mouseover(d, e) {
    ...
    if (e.pageX + 160 > $(this.refs.fullContainer).width()) {
      tooltipPos.right =
        $(this.refs.fullContainer).width() -
        (e.pageX - renderingAreaOffset.left) +
        15;
    } else {
  ...
  setFill(d) {
    if (d.data) {
      ...
      if (
        ...
        dataVal ===
        "Birth State of Jihadist Terrorist Responsible for Attack in U.S. and Trump Visa Restricted Country"
      ) {
        return "#927d85";
